...
module.exports.run = async (client, interaction) =>{
  ...
  const prefix = prefixDB.get(`Prefix_${client.user.id}_suggestion`)
  const owner = ownerDB.get(`Owner_${client.user.id}_suggestion`)
  const { commandName, options, user, guildId } = interaction;

  const command = await client.Suggestionbotsslashcommands.get(commandName)
  if (!command) return;
