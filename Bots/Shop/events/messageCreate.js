...
module.exports.run = async (client, message) => {
  const prefix = prefixDB.get(`Prefix_${client.user.id}_shop`)
  ...
  let command = client.Shopcommands.get(cmd) || client.Shopcommands.find(command => command.aliases && command.aliases.includes(cmd));
  if (!command) return;
