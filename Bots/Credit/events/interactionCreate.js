...
const client = require(`../Credit-bots`)
...
module.exports.run = async (client, interaction) =>{
  ...
  const owner = ownerDB.get(`Owner_${client.user.id}_credit`)
  const { commandName, options, user, guildId } = interaction;

  const command = await client.Creditbotsslashcommands.get(commandName)
  if (!command) return;
