...
module.exports.run = async (client, interaction) =>{
  ...
  const prefix = prefixDB.get(`Prefix_${client.user.id}_probot`)
  const owner = ownerDB.get(`Owner_${client.user.id}_probot`)

  const command = await client.Probotbotsslashcommands.get(commandName)
  if (!command) return;
