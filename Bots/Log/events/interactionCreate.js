...
module.exports.run = async (client, interaction) =>{
  ...
  const prefix = prefixDB.get(`Prefix_${client.user.id}_log`)
  const owner = ownerDB.get(`Owner_${client.user.id}_log`)
  const command = await client.Logbotsslashcommands.get(commandName)
  if (!command) return;
