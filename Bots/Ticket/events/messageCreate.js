...
module.exports.run = async (client, message) => {
  const prefix = prefixDB.get(`Prefix_${client.user.id}_ticket`)
  const owner = ownerDB.get(`Owner_${client.user.id}_ticket`)
  ...
  let command = client.Ticketcommands.get(cmd) || client.Ticketcommands.find(command => command.aliases && command.aliases.includes(cmd));
  if (!command) return;
