    ...
    run: async (client, message, args, config) => {
        try {
            ...
            if (hasRequiredRoles) {
                ...
                message.channel.setName(name).then(() => {
                    const doneEmbed = new Discord.MessageEmbed()
                        .setColor(`WHITE`)
                        .setDescription(`[✔] تم تغيير اسم الروم إلى **${name}** بنجاح`);
                    message.reply({ embeds: [doneEmbed] }).then((msg)=>{
                        message.delete().catch(err =>{})
                        setInterval(() => {
                            msg.delete().catch(err =>{})
                          }, 5000);
                          const check = ticketdb.get(`Changed_${message.channel.id}`) || 'false'
                          if(check === 'false'){
                              ticketdb.set(`${message.channel.id}`,message.channel.name)
                          }
                          ticketdb.set(`Changed_${message.channel.id}`,`true`)
                    })
                }).catch((error) => {
                    console.error(error);
