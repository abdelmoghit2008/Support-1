...
module.exports.run = async (client, interaction) => {
  if (interaction.isCommand()) {
    const prefix = prefixDB.get(`Prefix_${client.user.id}_scammer`)
    const owner = ownerDB.get(`Owner_${client.user.id}_scammer`)

    const { commandName, options, user, guildId } = interaction;

    const command = await client.ScammerBotsslashcommands.get(commandName)
    if (!command) return;
