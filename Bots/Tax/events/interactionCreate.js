...
module.exports.run = async (client, interaction) => {
  if (interaction.isCommand()) {
    const prefix = prefixDB.get(`Prefix_${client.user.id}_tax`)
    const owner = ownerDB.get(`Owner_${client.user.id}_tax`)
    const { commandName, options, user, guildId } = interaction;

    const command = await client.TaxBotsslashcommands.get(commandName)
    if (!command) return;
  ...
  //Button
    ...
    const button = await client.TaxButtons.get(interaction.customId);
    if (!button) return;
