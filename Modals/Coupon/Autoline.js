...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
        ...
        try {
            if(data){
                let Code = interaction.fields.getTextInputValue("Autoline_Redeem")
                ...
                if(da){
                    ...
                    const AutolinePrice = db3.get(`AutolineP_${interaction.guild.id}`) || db3.get(`AutolineP`) || 2;
                    const Autolinetotalprice = Math.floor(AutolinePrice * (1 - (discount / 100)) * (20 / 19) + 1);
