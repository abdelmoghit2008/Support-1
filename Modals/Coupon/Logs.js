...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
      ...
      try {
        if(data){
            let Code = interaction.fields.getTextInputValue("logs_Redeem")
            ...
            if(da){
                ...
                const LogPrice = db3.get(`LogP_${interaction.guild.id}`) || db3.get(`LogP`) || 2;
                const Logtotalprice = Math.floor(AutolinePrice * (1 - (discount / 100)) * (20 / 19) + 1);
