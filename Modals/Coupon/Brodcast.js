...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
      ...
      try {
        if(data){
            let Code = interaction.fields.getTextInputValue("Brodcast_Redeem")
            ...
            if(da){
                ...
                const BrodcastPrice = db3.get(`BrodcastP_${interaction.guild.id}`) || db3.get(`BrodcastP`) || 2;
                const Brodcasttotalprice = Math.floor(BrodcastPrice * (1 - (discount / 100)) * (20 / 19) + 1);
