...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
      ...
      try {
        if(data){
            let Code = interaction.fields.getTextInputValue("FeedBack_Redeem")
            ...
            if(da){
                ...
                const FeedbackPrice = db3.get(`FeedbackP_${interaction.guild.id}`) || db3.get(`FeedbackP`) || 2;
                const Feedbacktotalprice = Math.floor(FeedbackPrice * (1 - (discount / 100)) * (20 / 19) + 1);
