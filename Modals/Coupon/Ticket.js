...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
      ...
      try {
        if(data){
            let Code = interaction.fields.getTextInputValue("Ticket_Redeem")
            ...
            if(da){
                ...
                const TicketPrice = db3.get(`TicketP_${interaction.guild.id}`) || db3.get(`TicketP`) || 2;
                const Tickettotalprice = Math.floor(TicketPrice * (1 - (discount / 100)) * (20 / 19) + 1);
                ...
                await interaction.reply(
                    ...
                    \`\`\`c ${ownerID} ${Tickettotalprice}\`\`\``//تعديل
                )
