...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
      ...
      try {
        if(data){
            let Code = interaction.fields.getTextInputValue("Probot_Redeem")
            ...
            if(da){
                ...
                const ProbotPrice = db3.get(`ProbotP_${interaction.guild.id}`) || db3.get(`ProbotP`) || 2;
                const Probottotalprice = Math.floor(ProbotPrice * (1 - (discount / 100)) * (20 / 19) + 1);
