...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
      ...
      try {
        if(data){
            let Code = interaction.fields.getTextInputValue("Suggestion_Redeem")
            ...
            if(da){
                ...
                const SuggestionPrice = db3.get(`SuggestionP_${interaction.guild.id}`) || db3.get(`SuggestionP`) || 2;
                const Suggestiontotalprice = Math.floor(SuggestionPrice * (1 - (discount / 100)) * (20 / 19) + 1);
