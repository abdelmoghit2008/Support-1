...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
      ...
      try {
        if(data){
            let Code = interaction.fields.getTextInputValue("System_Redeem")
            ...
            if(da){
                ...
                const SystemPrice = db3.get(`SystemP_${interaction.guild.id}`) || db3.get(`SystemP`) || 2;
                const Systemtotalprice = Math.floor(SystemPrice * (1 - (discount / 100)) * (20 / 19) + 1);
