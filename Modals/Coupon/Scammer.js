...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
      ...
      try {
        if(data){
            let Code = interaction.fields.getTextInputValue("Scammer_Redeem")
            ...
            if(da){
                ...
                const ScammerPrice = db3.get(`ScammerP_${interaction.guild.id}`) || db3.get(`ScammerP`) || 2;
                const Scammertotalprice = Math.floor(ScammerPrice * (1 - (discount / 100)) * (20 / 19) + 1);
