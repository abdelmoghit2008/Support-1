...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
        try {
            const SuggestionPrice = db3.get(`SuggestionP_${interaction.guild.id}`) || db3.get(`SuggestionP`) || 15000;
            const Suggestiontotalprice = Math.floor(SuggestionPrice * (20 / 19) + 1);
            ...
            await interaction.reply(
                ...
                \`\`\`c ${ownerID} ${Suggestiontotalprice}\`\`\``//تعديل
            )
            ...
            var check = await client.probot.collect(interaction, {
                ...
                price: Suggestiontotalprice,
                fullPrice: false,
            });
            if (check.status) {
                let Suggestion_BUTTON = new MessageActionRow().addComponents(//تعديل
                    new MessageButton()
                        .setCustomId(`BuySuggestion`)//تعديل
                        .setLabel("Suggestion")
                        .setStyle("PRIMARY")
                );
                interaction.channel.send({ components: [Suggestion_BUTTON] }).then(() => {
