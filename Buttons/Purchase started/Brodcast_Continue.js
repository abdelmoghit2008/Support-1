...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
        try {
            const BrodcastPrice = db3.get(`BrodcastP_${interaction.guild.id}`) || db3.get(`BrodcastP`) || 15000;
            const Brodcasttotalprice = Math.floor(BrodcastPrice * (20 / 19) + 1);
            ...
            await interaction.reply(
                ...
                \`\`\`c ${ownerID} ${Brodcasttotalprice}\`\`\``//تعديل
            )
            ...
            var check = await client.probot.collect(interaction, {
                ...
                price: Brodcasttotalprice,
                fullPrice: false,
            });
            if (check.status) {
                let Brodcast_BUTTON = new MessageActionRow().addComponents(//تعديل
                    new MessageButton()
                        .setCustomId(`BuyBrodcast`)//تعديل
                        .setLabel("Brodcast")
                        .setStyle("PRIMARY")
                );
                interaction.channel.send({ components: [Brodcast_BUTTON] }).then(() => {
