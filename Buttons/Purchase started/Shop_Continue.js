...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
        try {
            const ShopPrice = db3.get(`ShopP_${interaction.guild.id}`) || db3.get(`ShopP`) || 15000;
            const Shoptotalprice = Math.floor(ShopPrice * (20 / 19) + 1);
            ...
            await interaction.reply(
                ...
                \`\`\`c ${ownerID} ${Shoptotalprice}\`\`\``//تعديل
            )
            ...
            if (check.status) {
                let Shop_BUTTON = new MessageActionRow().addComponents(//تعديل
                    new MessageButton()
                        .setCustomId(`BuyShop`)//تعديل
                        .setLabel("Shop")
                        .setStyle("PRIMARY")
                );
                interaction.channel.send({ components: [Shop_BUTTON] }).then(() => {
