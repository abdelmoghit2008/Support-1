...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
        try {
            const LogPrice = db3.get(`LogP_${interaction.guild.id}`) || db3.get(`LogP`) || 15000;
            const Logtotalprice = Math.floor(LogPrice * (20 / 19) + 1);
            ...
            if (check.status) {
                let Log_BUTTON = new MessageActionRow().addComponents(//تعديل
                    new MessageButton()
                        .setCustomId(`BuyLog`)//تعديل
                        .setLabel("Log")
                        .setStyle("PRIMARY")
                );
                interaction.channel.send({ components: [Log_BUTTON] }).then(() => {
