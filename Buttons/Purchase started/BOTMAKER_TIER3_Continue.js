    ...
    run: async (client, interaction, args, config) => {
        try {
          ...
          if(interaction.guild.id !== CoderServer && amount <=0){
            ...
            const BOTMAKERTier1Price = db3.get(`BOTMAKERP_Tier3`) || 15000;
            ...
            await interaction.reply(
              `__***.قم بكتابه امر التحويل التالي***__
                    ...
                    \`\`\`c ${ownerID} ${BOTMAKETier1Rtotalprice}\`\`\``//تعديل
            )
            ...
            if (check.status) {
              let BOTMAKER_BUTTON = new MessageActionRow().addComponents(//تعديل
                new MessageButton()
                  .setCustomId(`BOT_MAKER_Tier3_SUBBUTTON`)//تعديل
                  .setLabel("Subscribe")
                  .setStyle("PRIMARY")
              );
