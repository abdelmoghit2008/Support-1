...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
        try {
            const SystemPrice = db3.get(`SystemP_${interaction.guild.id}`) || db3.get(`SystemP`) || 15000;
            const Systemtotalprice = Math.floor(SystemPrice * (20 / 19) + 1);
            ...
            await interaction.reply(
                ...
                \`\`\`c ${ownerID} ${Systemtotalprice}\`\`\``//تعديل
            )
            ...
            var check = await client.probot.collect(interaction, {
                ...
                price: Systemtotalprice,
                fullPrice: false,
            });
            if (check.status) {
                let System_BUTTON = new MessageActionRow().addComponents(//تعديل
                    new MessageButton()
                        .setCustomId(`BuySystem`)//تعديل
                        .setLabel("System")
                        .setStyle("PRIMARY")
                );
                interaction.channel.send({ components: [System_BUTTON] }).then(() => {
