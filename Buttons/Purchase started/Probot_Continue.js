...
module.exports = {
    ...
    run: async (client, interaction, args, config) => {
        try {
            const ProbotPrice = db3.get(`ProbotP_${interaction.guild.id}`) || db3.get(`ProbotP`) || 15000;
            const Probottotalprice = Math.floor(ProbotPrice * (20 / 19) + 1);
            ...
            await interaction.reply(
                ...
                \`\`\`c ${ownerID} ${Probottotalprice}\`\`\``//تعديل
            )
            ...
            var check = await client.probot.collect(interaction, {
                ...
                price: Probottotalprice,
                fullPrice: false,
            });
            if (check.status) {
                let Probot_BUTTON = new MessageActionRow().addComponents(//تعديل
                    new MessageButton()
                        .setCustomId(`BuyProbot`)//تعديل
                        .setLabel("Probot")
                        .setStyle("PRIMARY")
                );
                interaction.channel.send({ components: [Probot_BUTTON] }).then(() => {
